...
const initialValue = {[Types.PLAYLIST]: {count: 0}, [Types.SONG]: {count: 0}, [Types.LINEUP]: {count: 0}}

export const useData = (): {infos: Record<Types, Entry>, loadData: () => Promise<void>, clear: () => void} => {
    ...
    useEffect(() => {
        setInfos(cur => ({
            ...
            [Types.SONG]: {...cur[Types.SONG],count: playlists.filter(({fetched}) => fetched).length}, 
    ...
    const loadData = async () => {
        ...
        setInfos((infos) => ({...infos, [Types.PLAYLIST]: {...infos[Types.PLAYLIST], loading: true}}))
        await fetchAllPlaylists()
        setInfos((infos) => ({...infos, [Types.PLAYLIST]: {...infos[Types.PLAYLIST], finished: true, loading: false}}))
        setInfos((infos) => ({...infos, [Types.SONG]: {...infos[Types.SONG], loading: true}}))
        await fetchAllSongsOfAllPlaylists()
        setInfos((infos) => ({...infos, [Types.SONG]: {...infos[Types.SONG], finished: true, loading: false}}))
        setInfos((infos) => ({...infos, [Types.LINEUP]: {count: Object.values(data).length, loading: true}}))
        const fetchedData = await getData();
        setData(fetchedData);
        setInfos((infos) => ({...infos, [Types.LINEUP]: {count: Object.values(data).length, finished: true, loading: false}}))
    }
    ...
    const fetchAllSongsOfAllPlaylists = async() => {
        ...
        for (let i = 0; i < playlistsToFetch.length; ) {
            ...
            setPlaylists(cur => cur.map((curPlaylist) => curPlaylist.id === id ? {...curPlaylist, fetched: new Date()} : curPlaylist))
            i++
        }
    }
