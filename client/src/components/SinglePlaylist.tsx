...
export const Playlist = ({id, name, tracks: numberOfSongs, toggle, hideSongs, fetching}: PlaylistProps) => {
    ...
    const setExcludedPlaylistIdWrapper = (event: React.MouseEvent<HTMLInputElement>) => {
    ...
    const artists = songs.reduce((acc, {artists}) => [...acc, ...artists.map(({id}) => id)], [] as string[])
    const containsFocusedArtist = focused?.type === "artist" && artists.includes(focused.id)
            ...
            <div className="playlist_song-container">
                {!hideSongs && <div>
                    {songs.map(({name, id, artists}) => {
                        const isFocused = focused?.type === "artist" && artists.map(({id}) => id).includes(focused.id)
                        return (
                            <div className={isFocused ? "focused playlist_song" : "playlist_song"} key={id}>"{name}" - {artists.map(({ name }) => name).join(', ')}</div>
                        )
                    })}
                </div>}
            </div>
