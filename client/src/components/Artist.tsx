...
export const Artist = ({
  ...
  const focusedPlaylistSongIds = (playlistSongs[focused?.id ?? ""] ?? []).map(
    ({ id }) => id,
  );
  const containsSongFromFocusedPlaylist =
    focused?.type === "playlist" &&
    focusedPlaylistSongIds.some((id) =>
      tracks.map(({ id: trackId }) => trackId).includes(id),
    );
  ...
  return (
    <div
      className={
        isFocused || (containsSongFromFocusedPlaylist && !foldedOut)
          ? " focused artist"
          : "artist"
      }
      key={id}
      onClick={setFoldedOut}
    >
      {name}{" "}
      {foldedOut ? (
          ...
          <div className="track-list">
            {tracks.map((song) => (
              <div
                key={song.id}
                className={
                  focusedPlaylistSongIds.includes(song.id)
                    ? "song focused"
                    : "song"
                }
              >
